...
def karatsuba_multiplication(x: int, y: int) -> int:
    """Takes two integers, multiplies them and returns the result as an integer.
    ...
    second_halves_multiplication = karatsuba_multiplication(
        second_half_x, second_half_y
    )
    sum_multiplication = karatsuba_multiplication(
        first_half_x + second_half_x, first_half_y + second_half_y
    )

    intermediate_numbers = (
        sum_multiplication - first_halves_multiplication - second_halves_multiplication
    )

    return (
        (10 ** (2 * n2)) * round(first_halves_multiplication)
        + (10 ** (n2)) * round(intermediate_numbers)
        + round(second_halves_multiplication)
    )
